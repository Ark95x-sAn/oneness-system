...
function Get-Arko95MissionControlPaths {
    ...
    [pscustomobject]@{
        ...
        AdapterObservations = Join-Path $candidate 'adapter-observations'
    }
}
...
function Get-Arko95MissionControlPolicy {
    ...
    $allowed = @($policy.allowed_operations_capabilities | Sort-Object)
    $compiled = @($opsPolicy.automatic_capabilities | ForEach-Object { [string]$_.id } | Sort-Object)
    if (($allowed -join '|') -cne ($compiled -join '|')) { throw 'Mission Control may route only the exact compiled Operations VP capabilities.' }
    $openClaw = @($policy.adapters | Where-Object { $_.id -eq 'openclaw_companion' })
    if ($openClaw.Count -ne 1 -or $openClaw[0].effect -ne 'proposal_only' -or $openClaw[0].may_write_mission_events -ne $false -or $openClaw[0].may_invoke_operations -ne $false -or $openClaw[0].may_approve -ne $false -or $openClaw[0].may_clear_kill_latch -ne $false) {
        throw 'OpenClaw must remain a proposal/status adapter without mission authority.'
    }
    return $policy
}
...
function Initialize-Arko95MissionControlState {
    ...
    foreach ($directory in @($Paths.Root,$Paths.Artifacts,$Paths.Presentations,$Paths.AdapterObservations)) {
