...
if (@($policy.allowed_operations_capabilities).Count -ne 5) { throw 'Mission Control no longer binds exactly five Operations VP capabilities.' }
$openClaw = @($policy.adapters | Where-Object { $_.id -eq 'openclaw_companion' })[0]
if ($openClaw.effect -ne 'proposal_only' -or $openClaw.may_write_mission_events -ne $false -or $openClaw.may_invoke_operations -ne $false -or $openClaw.may_approve -ne $false -or $openClaw.may_clear_kill_latch -ne $false) {
    throw 'OpenClaw gained mission, execution, approval, or kill authority.'
}
if ($policy.sensor_acceptance.catalog_entry_is_availability -ne $false -or $policy.sensor_acceptance.unverified_state -ne 'unavailable') {
    throw 'A sensor catalog entry is being treated as live capability.'
}
...
try {
    ...
    foreach ($configName in @('capabilities.json','connector-registry.json','delegation-roster.json','mission-control.json','operations-vp.json')) {
