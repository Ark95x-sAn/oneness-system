...
if ($toolIndex.TotalCount -ne 13 -or $toolIndex.ConnectorCount -ne 9 -or $toolIndex.AdapterCount -ne 4 -or $toolIndex.Authority -ne 'none' -or $toolIndex.ProjectionOnly -ne $true) {
    throw 'The unified tool index lost its 9+4 projection-only, zero-authority boundary.'
}
if (@($toolIndex.Entries | Where-Object { $_.effect -ne 'proposal_only' -or $_.authority -ne 'none' -or $_.auto_invoke -ne $false }).Count -ne 0) {
    throw 'A unified tool-index entry gained invocation or authority.'
}
if ((@($toolIndex.Entries | Group-Object lane | Sort-Object Name | ForEach-Object { $_.Name }) -join '|') -cne 'runtime_status_adapter|specialist_routing_hint') {
    throw 'The unified view collapsed its separately governed tool lanes.'
}
