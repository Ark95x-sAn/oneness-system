...
Decision Learning is the sole canonical source for adapter definitions and current receipts under `state/decision-learning/adapter-observations`. Mission Control consumes only its zero-authority status view. The older OpenClaw receipt was removed from live state and preserved locally as legacy evidence with its original-path and SHA-256 manifest. OpenClaw cannot write mission events, select a sensor, invoke Operations VP, approve work, promote learning, or clear the kill latch.

Mission Control also derives its executable capability allowlist directly from `config/operations-vp.json`; it no longer keeps a second copy that can drift.
